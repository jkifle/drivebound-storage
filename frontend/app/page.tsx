...
export default function LandingPage() {
  return <main className="landing-page" id="main-content">
    <header className="landing-nav">
      <Link className="brand" href="/"><span className="brand-mark" aria-hidden="true">D</span><span>Drivebound</span></Link>
    ...
    <section className="landing-hero">
      <div data-reveal="left"><span className="eyebrow">Storage without the ceiling</span><h1>Your photos online. Your drives underneath.</h1><p>Drivebound turns storage you own into a private photo cloud you can reach anywhere—without locking your memories behind another monthly capacity tier.</p><div className="hero-actions"><Link className="primary-button" href="/auth?mode=register">Start with your first drive</Link><a className="secondary-button" href="#how">See how it works</a></div><div className="trust-row" aria-label="Drivebound benefits"><span>✓ Private accounts</span><span>✓ Verified originals</span><span>✓ Expandable storage</span></div></div>
      <div className="landing-visual" data-reveal="right"><img src="/og-neumorphic.png" alt="Drivebound library connected to expandable drives" width="1536" height="1024" /></div>
    </section>
    ...
    <section className="landing-security" id="security" data-reveal><div className="step-icon success">✓</div><div><span className="eyebrow">Built around ownership</span><h2>Your account is the boundary.</h2><p>Every asset, album, device, upload, and drive lookup is filtered by the signed-in account. Sessions are revocable, security events are visible, password recovery uses single-use email links, and storage recovery remains under the host administrator&apos;s control.</p></div><Link className="secondary-button" href="/auth">Sign in securely</Link></section>
    <footer data-reveal><div className="brand"><span className="brand-mark" aria-hidden="true">D</span>Drivebound</div><p>Your storage. Your cloud.</p></footer>
  </main>;
}
